...
export default function DestinationDetailPage({ params }: { params: { id: string } }) {
  ...
  const getRelatedDestinations = () => {
    let relatedDests = destinations.filter(d => d.id !== id);
    
    if (relatedFilter === "region") {
      // Lọc theo cùng khu vực
      const currentDestination = destinations.find(d => d.id === id);
      if (currentDestination) {
        const region = currentDestination.region;
        relatedDests = destinations.filter(d => d.id !== id && d.region === region);
      }
      ...
      if (currentDestination) {
        ...
        relatedDests = destinations.filter(
          d => d.id !== id && Math.abs(d.priceNumeric - price) <= 1000000
        );
      }
    }
